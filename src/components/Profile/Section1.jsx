...
export default function Section1({ profile }) {
  ...
  return (
    <motion.div   animate={{opacity: 1 }}  initial={{ opacity: 0 }} transition={{ease: "easeOut", duration: 2}} className="flex flex-col w-full bg-accent md:bg-[bottom_left_6rem] bg-no-repeat md:bg-[url('/images/about-us/house.png')]">
      ...
      <Modal id={"contratar"} className="w-full mx-2 md:w-8/12 max-w-5xl bg-[#F0ECF1]">
         <a href="#" className="btn btn-sm btn-ghost btn-circle absolute right-2 top-2">✕</a>
        <div className="flex flex-col h-full w-full">
          <div className="flex justify-center w-full">
            <TextTracking>
              VOCê ESCOLHEU {selectedProfile.name}
            </TextTracking>
          </div>
          <div className="flex flex-col-reverse lg:flex-row py-2">
            <div className="flex flex-col flex-1 gap-3 py-4">
              ...
              <span className='font-bold'>Entraremos em contato para confirmar a o perfil escolhido.</span>

              <div className="flex flex-col w-full">
                  <ModalSection title={"Categoria"}>
                      Diarista
                  </ModalSection>
              </div>
              <div className="flex flex-col w-full">
                  <ModalSection title={"Serviços"}>
                    <ul className="flex flex-col">
                      {selectedProfile.services.map((service, index) => <li key={index}>{service}</li> )}
                    </ul>
                  </ModalSection>
              </div>
              <div className="flex justify-between gap-8 ">
                ...
                <ModalSection title={"Cômodos"}>
                    <ul className="flex flex-col">
                      {selectedProfile.comodos.map((service, index) => <li key={index}>{service}</li> )}
                    </ul>
                </ModalSection>
              </div>
              ...
              <div className="flex flex-col gap-3">
                <CheckBox label="Confirmo todas informações a cima" labelClass="font-bold" checkboxClass="checkbox-primary"/>
                <CheckBox label="Quero refazer" labelClass="font-bold" checkboxClass="checkbox-primary"/>
              </div>
            </div>
            <div className='flex flex-col w-auto'>
              <img src={selectedProfile.image} className="max-w-xs self-center max-h-96"/>
            </div>
          </div>
          <div className="flex flex-col w-full pt-8">
            <span> Nos informe as demais informações para entamos em contato</span>
